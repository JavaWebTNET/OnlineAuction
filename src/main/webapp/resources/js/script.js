...
function valreg(id,ider,field){
	var name=getid(id).val().trim();
	if(name.length>=6 && name.length<=15){
		var regex=/^[a-zA-Z0-9]+$/;
		if (!regex.test(name)){
			getid(ider).html(field+" must be alphanumeric with no spaces");
			return false;
		}
		getid(ider).html("");
		getid(id).val(name);
		...
		getid(ider).html(field+" cannot empty");
		return false;
	}
	else{
		getid(ider).html(field+" must be between 6 and 15 characters long");
		return false;
	}
...
function valrepass(){
	var pass=getid("ippass").val().trim();
	var repass=getid("iprps").val().trim();
	if(repass!=pass){
		getid("rpser").html("Password not the same");
		return false;
	}
	getid("rpser").html("");
	return true;
...
function valemail(){
	var email=getid("ipemail").val().trim();
	if(email.length>0){
		var regex=/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/;
		if (!regex.test(email)){
			getid("emailer").html("Email Invalid");
			return false;
		}
		getid("emailer").html("");
		getid("ipemail").val(email);
		return true;
	}
	getid("emailer").html("Email cannot empty");
	return false;
}
function chkformrs(){
	chkid=valid();
...
function searchtoggle(){
	getid("search-container").toggleClass('hide');
	getid("sf-s").focus();	
}
function menutoggle(){	
	getid("primary-navigation").toggleClass("toggled-on");		
}
function togglecat(id){
	getid(id).toggleClass("cat_item_children");
}
